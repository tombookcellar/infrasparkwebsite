---
title: "About"
subtitle: "Infrastructure practitioner. Change leader."
layout: "about"
profile_name: "Dr Tom Bartley"
profile_credentials: "EngD AMICE"
profile_role: "Founder, InfraSpark"
---

I have spent my career delivering technology-enabled change in infrastructure. As a civil engineer, technologist and change practitioner, I understand the ingredients required to embed new capabilities in complex delivery environments.

I have worked for client, designers and contractors across transport, water, energy and commercial buildings; delivering lean exercises, ERP transformations, 3D modelling programmes, standards development, PMO redesign, data analytics, structures inspection. The same pattern emerges again and again — technically sound initiatives that fail to land, innovations that never scale, technology investments that leave the way people actually work largely unchanged.

That gap between what technology promises and what organisations actually capture is where I work.

The framework I have developed draws on the best thinking in constraint management, lean strategy, product risk, and stakeholder alignment — applied specifically to the dynamics of infrastructure programmes, where every project is different, every supply chain is new, and institutional complexity is the default condition.

I also run [the BookCellar](https://bookcellar.uk), an independent bookshop and reading room in Bristol. The reading list that underpins this work is available there.

---

**Technologies and domains**

- Project controls
- Programme management
- BIM and digital construction
- Infrastructure asset management
- Technical standards & requirements
- AI and machine learning
- Lean and process improvement
- Agile and Scrum
- Product management
- Data and analytics
- ERP and enterprise systems

---

[The Framework](/framework/) · [The Transformation Worksheet](/worksheet/) · [Work With Me](/work-with-me/)
